...
class NavRLEnvX(habitat.RLEnv):
    ...
    def get_reward(self, observations):
        ...
        if self._episode_success():
            reward += self._episode_success() * 5
            print(f"Success: {self._env.get_metrics()['success']}, SPL: {self._env.get_metrics()['spl']}",
                  f"distance: {self._env.get_metrics()['distance_to_goal']}, view_angle: {self._env.get_metrics()['view_angle']}",
                  f"distance_to_view: {self._env.get_metrics()['distance_to_view']}")
