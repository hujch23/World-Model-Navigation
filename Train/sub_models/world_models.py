...
class DistHead(nn.Module):
    ...
    def __init__(self, image_feat_dim, transformer_hidden_dim, stoch_dim) -> None:
        ...
        # self.post_head_view = nn.Linear(image_feat_dim_view, stoch_dim * stoch_dim)
    ...
    def forward_post_view(self, x):
        logits = self.post_head_view(x)
        logits = rearrange(logits, "B L (K C) -> B L K C", K=self.stoch_dim)
        logits = self.unimix(logits)
        return logits
...
class WorldModel(nn.Module):
    def __init__(self, in_channels, action_dim,
        ...
        self.dist_head = DistHead(
            image_feat_dim=2*self.encoder.last_channels*self.final_feature_width*self.final_feature_width,
    ...
    def encode_obs(self, obs):
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            ...
            embedding_view = self.encoder(obs[:, :, :3, :, :])
            embedding_goal = self.encoder(obs[:, :, 3:6, :, :])
            embedding = torch.cat((embedding_view, embedding_goal), dim=-1)
    ...
    def calc_last_dist_feat_train(self, latent, action):
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            temporal_mask = get_subsequent_mask(latent)
            dist_feat = self.storm_transformer(latent, action, temporal_mask)
            ...
            prior_sample = self.stright_throught_gradient(prior_logits, sample_mode="random_sample")
            prior_flattened_sample = self.flatten_sample(prior_sample)
        return prior_flattened_sample, dist_feat
    ...
    def imagine_data(self, agent: agents.ActorCriticAgent, sample_obs, sample_action,
        ...
        for i in range(imagine_batch_length):
            ...
            # last_obs_hat = last_obs_hat[:, :, :3, :, :]
            # A = last_obs_hat[::imagine_batch_size // 16]
            # A = A.squeeze(1)
            # from torchvision.utils import save_image
            # for i in range(A.shape[0]):
            # # 每次保存一张图像，图像名为 img_{i}.png
            #     save_image(A[i], f'img_{i}.png')
        # if log_video:
    ...
    def update(self, obs, action, reward, termination, logger=None):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            ...
            # for t in range(4):  # 假设 obs 的形状是 [batch_size, time_steps, channels, height, width]
            #         # 记录时间步 t 的原始 RGB 图像（假设 obs 的通道数是 6）
            #         writer.add_image(f"WorldModel/original_image/{t}", obs[0, t, :3, :, :].cpu().to(torch.float32),
            #                          self.step)
            #
            #         # 记录时间步 t 的重建 RGB 图像（假设 obs_hat 只有 3 个通道，保持在0-1范围内）
            #         writer.add_image(f"WorldModel/reconstructed_image/{t}", obs_hat[0, t, :, :, :].cpu().to(torch.float32),
            #                          self.step)
            #
            # self.step += 1

            reconstruction_loss = self.mse_loss_func(obs_hat, obs[:, :, :3, :, :])
            reward_loss = self.symlog_twohot_loss_func(reward_hat, reward)
            termination_loss = self.bce_with_logits_loss_func(termination_hat, termination)
            dynamics_loss, dynamics_real_kl_div = self.categorical_kl_div_loss(post_logits[:, 1:].detach(), prior_logits[:, :-1])
            representation_loss, representation_real_kl_div = self.categorical_kl_div_loss(post_logits[:, 1:], prior_logits[:, :-1].detach())
            total_loss = reconstruction_loss + reward_loss + 0.5*dynamics_loss + 0.1*representation_loss + termination_loss
